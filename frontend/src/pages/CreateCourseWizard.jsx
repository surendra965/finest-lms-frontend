...
const CreateCourseWizard = () => {
    ...
    return (
        <div className="min-h-screen bg-gray-50 flex items-center justify-center p-4 font-sans">
            <div className="bg-white w-full max-w-2xl rounded-2xl shadow-xl border border-gray-100 p-10 flex flex-col justify-between min-h-[480px]">
                <div>
                    {/* PROGRESS BAR */}
                            ...
                            <div className="space-y-3">
                                {["Course", "Practice Test"].map((type) => (
                                    <button
                                        ...
                                        className={`w-full py-4 text-center border-2 font-semibold transition duration-200 ${
                                            form.type === type
                            ...
                            <div className="space-y-2">
                                <input
                                    ...
                                    onChange={(e) => {
                                        ...
                                        if (/[0-9]/.test(val)) {
                                            setTitleError("Numbers are not allowed in the course title.");
                                            setValidationError("Course title must not contain numbers.");
                                        } else {
                                            setTitleError("");
                                            setValidationError("");
                                    ...
                                    className="w-full border-2 border-zinc-800 p-4 text-lg outline-none focus:border-[#a435f0] transition duration-200 rounded"
                                    placeholder="e.g. Complete React Mastery"
                                    maxLength={80}
                                />
                                {titleError ? (
                                    <p className="text-xs text-red-500 font-semibold">{titleError}</p>
                                ) : (
                                        ...
                                        <button
                                            ...
                                            className={`w-full py-4 text-center border-2 font-semibold transition duration-200 ${
                                                form.category === cat.name
                                                    ? "border-[#a435f0] bg-purple-50/30 text-[#a435f0]"
                                                    : "border-zinc-800 bg-white text-zinc-800 hover:bg-zinc-50"
                                            }`}
                                        >
                                            {cat.name}
                                        </button>
                    ...
                    {step === 4 && (
                        <div className="space-y-6">
                            ...
                            <div className="space-y-2">
                                ...
                                />
                                <p className="text-xs text-gray-500">
                                    Tell us how many hours per week you can dedicate to building this course.
                                </p>
                            </div>
                        </div>
                    )}
